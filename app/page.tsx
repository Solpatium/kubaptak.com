...
const LinkButton: React.FC<{ children: string, href: string }> = (props) => {
  return <a href={props.href} className="block md:inline-block bg-gray-900 text-white text-center text-[24px] py-[16px] lg:py-[8px] px-[32px] hover:underline">{props.children}</a>
}
...
const LeftColumn = () => {
  return <aside className="lg:max-w-[500px] w-full relative">
    <div className="lg:sticky top-0 flex flex-col md:flex-row lg:flex-col gap-[32px] md:gap-[64px]">
      <div className="relative shrink-0">
        ...
        />
        {/* Big */}
        <Blob scale={1.5} className="md:hidden absolute top-[-250px] left-[-100px] md:top-5 md:left-0 lg:top-[50px] lg:left-[150px] z-[-1]" />
        {/* Small */}
...
export default function Home() {
  return (
    <div className="flex flex-col lg:flex-row justify-between gap-[64px] max-w-[1400px] m-auto py-8 lg:py-16 px-6 overflow-hidden">
      <LeftColumn />
      <main className="lg:max-w-[780px] w-full">
        <h2 className="text-[32px] md::text-[40px] font-serif mb-[24px] md:mb-[32px]">My projects</h2>
        <GreatResume />
      </main>
    </div>
  );
